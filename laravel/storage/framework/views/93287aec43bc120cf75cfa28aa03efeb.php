...
<head>
    ...
    <style>
        ...
        #camera-feed { transform: scaleX(-1); /* Mirror effect */ }
        .corner-tl { position:absolute; top:-3px; left:-3px; width:24px; height:24px; border-top:3px solid white; border-left:3px solid white; border-radius:4px 0 0 0; }
        .corner-tr { position:absolute; top:-3px; right:-3px; width:24px; height:24px; border-top:3px solid white; border-right:3px solid white; border-radius:0 4px 0 0; }
        .corner-bl { position:absolute; bottom:-3px; left:-3px; width:24px; height:24px; border-bottom:3px solid white; border-left:3px solid white; border-radius:0 0 0 4px; }
        .corner-br { position:absolute; bottom:-3px; right:-3px; width:24px; height:24px; border-bottom:3px solid white; border-right:3px solid white; border-radius:0 0 4px 0; }
    </style>
</head>
<body class="flex items-center justify-center p-4 py-8">
    ...
    <div class="w-full max-w-md">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-5 mb-4">
            ...
            <select id="session-select"
                ...
                <?php $__currentLoopData = $openSessions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $session): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($session->id); ?>">
                        <?php echo e($session->title); ?> | <?php echo e($session->classRoom->name); ?>
            ...
            <?php if($openSessions->isEmpty()): ?>
                <div class="mt-3 flex items-center gap-2 px-3 py-2.5 bg-amber-50 rounded-lg border border-amber-200">
                    ...
                    <p class="text-xs text-amber-700">
                        ...
                        <a href="<?php echo e(route('login')); ?>" class="underline font-medium">Hubungi admin.</a>
                    </p>
                </div>
            <?php endif; ?>
        </div>
            ...
            <div class="p-5">
                ...
                <div id="feedback-area" class="mb-4 min-h-[3rem] flex items-center justify-center">
                    ...
                    <p id="default-hint" class="text-xs text-gray-400 text-center">
                        Posisikan wajah di dalam bingkai, lalu tekan tombol presensi
                    </p>
                </div>
                ...
                <button id="capture-btn"
                        ...
                        disabled
                        class="w-full py-4 rounded-xl text-white font-bold text-base transition-all
                ...
                <p class="text-center mt-4 text-xs text-gray-400">
                    Admin?
                    <a href="<?php echo e(route('login')); ?>" class="text-blue-600 hover:underline">Masuk ke dashboard →</a>
                </p>
            </div>
        </div>
        ...
        <div class="text-center mt-4">
            ...
            <p class="text-blue-200 text-xs mt-0.5"><?php echo e(now()->isoFormat('dddd, D MMMM Y')); ?></p>
        </div>
    </div>
    ...
    document.getElementById('session-select').addEventListener('change', updateBtnState);
    ...
    function showFeedback(type, html) {
        ...
        msg.innerHTML = html;
        msg.className = `w-full px-4 py-3 rounded-xl text-sm text-center font-medium leading-relaxed ${
            type === 'success'
...
<?php /**PATH D:\PROJECT OOAD\attendance-system\laravel\resources\views/attendance/index.blade.php ENDPATH**/ ?>