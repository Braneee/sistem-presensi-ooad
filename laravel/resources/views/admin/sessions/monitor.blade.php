...
@section('page-title', 'Monitor Kehadiran Realtime')
@section('page-subtitle', $session->title . ' — ' . $session->classRoom->name)
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <dl class="space-y-2.5 text-sm">
                <div class="flex justify-between">
                    ...
                    <dd class="font-medium text-gray-800">{{ $session->classRoom->name }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-gray-500">Tanggal</dt>
                    <dd class="font-medium text-gray-800">{{ $session->date->isoFormat('D MMMM Y') }}</dd>
                </div>
                <div class="flex justify-between">
                ...
                <div class="flex justify-between">
                    ...
                    <dd class="font-mono text-xs bg-gray-100 px-2 py-0.5 rounded">{{ $session->code }}</dd>
                </div>
                @if($session->notes)
                    <div>
                        <dt class="text-gray-500 mb-1">Catatan</dt>
                        <dd class="text-gray-700 text-xs">{{ $session->notes }}</dd>
                    </div>
                @endif
            </dl>
            ...
            @if($session->status === 'open')
                <div class="mt-4 pt-4 border-t border-gray-100 flex gap-2">
                    <form action="{{ route('admin.sessions.close', $session) }}" method="POST" class="flex-1">
                        @csrf @method('PATCH')
                        <button type="submit"
            ...
            @else
                <div class="mt-4 pt-4 border-t border-gray-100 flex gap-2">
                    <form action="{{ route('admin.sessions.reopen', $session) }}" method="POST" class="flex-1">
                        @csrf @method('PATCH')
                        <button type="submit"
